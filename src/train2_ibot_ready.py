...
from models.ibot_vit_video import build_ibot_vit_video_classifier
...
def build_model(cfg: DictConfig) -> nn.Module:
    ...
    if name == "ibot_vit_video":
        return build_ibot_vit_video_classifier(
            num_classes=num_classes,
            num_frames=int(cfg.dataset.num_frames),
            pretrained=pretrained,
            vit_name=str(cfg.model.get("vit_name", "vit_base_patch16_224")),
            ibot_checkpoint=cfg.model.get("ibot_checkpoint", None),
            freeze_encoder=bool(cfg.model.get("freeze_encoder", False)),
            unfreeze_last_n_blocks=int(cfg.model.get("unfreeze_last_n_blocks", 0)),
            embed_dim=int(cfg.model.get("embed_dim", 768)),
            temporal_depth=int(cfg.model.get("temporal_depth", 4)),
            temporal_heads=int(cfg.model.get("temporal_heads", 12)),
            temporal_mlp_ratio=float(cfg.model.get("temporal_mlp_ratio", 4.0)),
            num_query_tokens=int(cfg.model.get("num_query_tokens", 4)),
            dropout=float(cfg.model.get("dropout", 0.30)),
            drop_path_rate=float(cfg.model.get("drop_path_rate", 0.10)),
            head_hidden_mult=float(cfg.model.get("head_hidden_mult", 1.5)),
            temporal_mask_prob=float(cfg.model.get("temporal_mask_prob", 0.05)),
        )
    ...
    if name == "video_swin":
        return build_video_swin_transformer_classifier(
            ...
            dropout=float(cfg.model.get("dropout", 0.45)),
            head_hidden_mult=float(cfg.model.get("head_hidden_mult", 1.0)),
...
def make_optimizer(cfg: DictConfig, model: nn.Module) -> torch.optim.Optimizer:
    """AdamW with no-decay groups and optional differential LR for video_swin.

    For Video Swin, the pretrained backbone should move slowly while the new
    classification head learns faster. This is one of the most important changes
    for Kaggle performance.
    """
    ...
    use_diff_lr = model_name in ("video_swin", "ibot_vit_video") and bool(cfg.training.get("differential_lr", True))
    backbone_lr_mult = float(cfg.training.get("backbone_lr_mult", 0.35)) if use_diff_lr else 1.0
    head_lr_mult = float(cfg.training.get("head_lr_mult", 4.0)) if use_diff_lr else 1.0
    ...
    for name, param in unwrap_model(model).named_parameters():
        ...
        part = "head" if is_head else "backbone"
        decay = not (param.ndim <= 1 or name.endswith(".bias") or "norm" in name.lower())
        groups[(part, decay)].append(param)

    param_groups = []
    for part in ("backbone", "head"):
        lr_mult = head_lr_mult if part == "head" else backbone_lr_mult
        for decay in (True, False):
            params = groups[(part, decay)]
            if not params:
                continue
            param_groups.append(
                {
    ...
    if optimizer_name == "sgd":
        return torch.optim.SGD(param_groups, lr=base_lr, momentum=0.9, nesterov=True)
...
def main(cfg: DictConfig) -> None:
    ...
    # OmegaConf configs are often in struct mode, so use open_dict to add this key safely.
